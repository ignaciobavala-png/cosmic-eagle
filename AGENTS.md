...
| Stack | Next.js 16.2.11 + React 19.2.4 + Supabase + Tailwind + Resend + Zustand + Framer Motion + Playwright + TypeScript + Vercel + E-commerce |
| Estado | activo |
...
- `playwright-e2e-delivery` — **Playwright — E2E automatizado para entrega a clientes**
  Cuando hay que **mostrarle el producto o un flujo a la clienta** —en una videollamada, o como entregable— y…
...
- `mock-huerfano-sin-conectar` — **Auditoría — pantalla con datos de maqueta hardcodeados que nunca se conectó a los datos reales**
  Al revisar o auditar cualquier flujo **multi-pantalla** donde una pantalla posterior debería mostrar algo que…
...
- `woocommerce-store-api-headless` — **WooCommerce headless con el Store API**
  Cuando hay que armar una vidriera (Astro, Next, SvelteKit) contra un WordPress + WooCommerce que sigue siendo…
